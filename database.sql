...
CREATE TABLE IF NOT EXISTS `report_palace`.`reports_created` (
  ...
  UNIQUE INDEX `report_created_id_UNIQUE` (`report_created_id` ASC) ,
  INDEX `fk_reports_created_status_report_idx` (`state_report_id` ASC) ,
...
CREATE TABLE IF NOT EXISTS `report_palace`.`report_bodies` (
  ...
  UNIQUE INDEX `report_body_id_UNIQUE` (`report_body_id` ASC) ,
  INDEX `fk_report_bodies_reports_created1_idx` (`report_created_id` ASC) ,
...
DELIMITER $$
USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`reports_created_BEFORE_INSERT` BEFORE INSERT ON `reports_created` FOR EACH ROW
BEGIN
	IF new.report_created_id IS NULL THEN
		SET new.report_created_id = uuid();
	END IF;
END$$

USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`report_bodies_BEFORE_INSERT` BEFORE INSERT ON `report_bodies` FOR EACH ROW
BEGIN
	IF new.report_body_id IS NULL THEN
		SET new.report_body_id = uuid();
	END IF;
END$$
...
DELIMITER $$
USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`users_BEFORE_INSERT` BEFORE INSERT ON `users` FOR EACH ROW
BEGIN
...
USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`anomalies_BEFORE_INSERT` BEFORE INSERT ON `anomalies` FOR EACH ROW
BEGIN
	IF new.anomaly_id IS NULL THEN
		SET new.anomaly_id = uuid();
	END IF;
END$$

USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`configurations_BEFORE_INSERT` BEFORE INSERT ON `configurations` FOR EACH ROW
BEGIN
	IF new.configuration_id IS NULL THEN
		SET new.configuration_id = uuid();
	END IF;
END$$

USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`autoships_BEFORE_INSERT` BEFORE INSERT ON `autoships` FOR EACH ROW
BEGIN
	IF new.autoship_id IS NULL THEN
		SET new.autoship_id = uuid();
	END IF;
END$$

USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`report_headers_BEFORE_INSERT` BEFORE INSERT ON `report_headers` FOR EACH ROW
BEGIN
	IF new.report_header_id IS NULL THEN
		SET new.report_header_id = uuid();
	END IF;
END$$

USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`report_footers_BEFORE_INSERT` BEFORE INSERT ON `report_footers` FOR EACH ROW
BEGIN
	IF new.report_footer_id IS NULL THEN
		SET new.report_footer_id = uuid();
	END IF;
END$$

USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`schedules_BEFORE_INSERT` BEFORE INSERT ON `schedules` FOR EACH ROW
BEGIN
...
USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`tenants_BEFORE_INSERT` BEFORE INSERT ON `tenants` FOR EACH ROW
BEGIN
	IF new.tenant_id IS NULL THEN
		SET new.tenant_id = uuid();
	END IF;
END$$

USE `report_palace`$$
CREATE DEFINER = CURRENT_USER TRIGGER `report_palace`.`tenants_configurations_BEFORE_INSERT` BEFORE INSERT ON `tenants_configurations` FOR EACH ROW
BEGIN
	IF new.tenant_configuration_id IS NULL THEN
		SET new.tenant_configuration_id = uuid();
	END IF;
END$$
